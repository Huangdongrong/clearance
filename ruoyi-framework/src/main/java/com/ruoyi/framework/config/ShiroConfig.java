...
public class ShiroConfig {
    ...
    public OnlineWebSessionManager sessionManager() {
        ...
        // session检查间隔
        manager.setSessionValidationInterval(validationInterval);
        // 定义要使用的无效的Session定时调度器
        manager.setSessionValidationScheduler(getBean(SpringSessionValidationScheduler.class));
        // 是否定时检查session
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(SecurityManager securityManager) {
        ...
        if (isNotBlank(anons)) {
            String[] anonsAry = split(anons, ",");
            for (String anon : anonsAry) {
                if (isNotBlank(anon)) {
