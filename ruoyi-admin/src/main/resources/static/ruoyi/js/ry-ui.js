...
(function ($) {
    $.extend({
        ...
        form: {
            ...
            fill: function (json) {
                ...
                $.each(json, function (name, ival) {
                    var $oinput = that.find("input:[name=" + name + "]");
                    if ($oinput.attr("type") == "radio" || $oinput.attr("type") == "checkbox") {
                        ...
                        that.find("[name=" + name + "]").html(ival);
                    } else {
                        that.find("[name=" + name + "]").val(ival);
                    }
                });
            }
        },
        ...
        yzorder: {
            // 清关
            execute: function (id) {
                $.modal.confirm("确定发起该条" + $.table._option.modalName + "数据清关吗？", function () {
                    var url = $.common.isEmpty(id) ? $.table._option.executeUrl : $.table._option.executeUrl.replace("{id}", id);
            ...
            executeAll: function () {
                ...
                $.modal.confirm("确认要发起选中的" + rows.length + "条数据清关吗?", function () {
                    var url = $.table._option.executeUrl;
        ...
        operate: {
            ...
            // async post请求传输
            asynPost: function (url, data, callback) {
                if (url) {
                    var config = {
                        url: url,
                        type: "post",
            ...
            // 废弃信息
            discard: function (id) {
                $.modal.confirm("确定废弃该条" + $.table._option.modalName + "数据吗？", function () {
                    var url = $.common.isEmpty(id) ? $.table._option.discardUrl : $.table._option.discardUrl.replace("{id}", id);
                    if ($.table._option.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = {"ids": id};
            ...
            discardAll: function () {
                ...
                $.modal.confirm("确认要废弃选中的" + rows.length + "条数据吗?", function () {
                    var url = $.table._option.discardUrl;
                    var data = {"ids": rows.join()};
                    $.operate.submit(url, "post", "json", data);
                });
            },
            ...
            recovery: function (id) {
                $.modal.confirm("确定重新恢复该条" + $.table._option.modalName + "数据吗？", function () {
                    var url = $.common.isEmpty(id) ? $.table._option.recoveryUrl : $.table._option.recoveryUrl.replace("{id}", id);
                    if ($.table._option.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
                        var data = {"ids": id};
            ...
            recoveryAll: function () {
                ...
                $.modal.confirm("确认要重新恢复选中的" + rows.length + "条数据吗?", function () {
                    var url = $.table._option.recoveryUrl;
                    var data = {"ids": rows.join()};
                    $.operate.submit(url, "post", "json", data);
                });
            },
            ...
            successTabCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var topWindowArray = $('.RuoYi_iframe[data-id="' + currentId + '"]', topWindow);
                    var $contentWindow = null;
                    if (topWindowArray && topWindowArray.length > 0) {
                        $contentWindow = topWindowArray[0].contentWindow;
                    }
                    if ($contentWindow) {
                        $.modal.close();
